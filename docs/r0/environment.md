...
- WHP result: `AZURE_WHP_BLOCKED`; the direct Custom Script Extension probe
  returned `WHvCapabilityCodeHypervisorPresent=false` twice, including after a
  clean deallocate/start. Overall R0 result is `R0_REQUIRES_BARE_METAL`, not a
  VMM rejection.
