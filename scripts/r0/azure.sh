...
case "${1:-preflight}" in
  preflight)
    ...
    az vm list-skus --location "$KODA_AZ_LOCATION" --resource-type virtualMachines \
      --size "$KODA_AZ_VM_SIZE" --all -o json
    az vm list-usage --location "$KODA_AZ_LOCATION" -o table
    az vm image list --location "$KODA_AZ_LOCATION" \
  ...
  create-gate)
    ...
    az group create --name "$KODA_AZ_RG" --location "$KODA_AZ_LOCATION" \
      --tags project=koda phase=r0-a disposable=true
    az vm create --resource-group "$KODA_AZ_RG" --name "$KODA_AZ_VM" \
    ...
    ;;
  deallocate)
    az vm deallocate --resource-group "$KODA_AZ_RG" --name "$KODA_AZ_VM"
    ;;
  delete)
    az group delete --name "$KODA_AZ_RG" --yes --no-wait
    ;;
  *)
